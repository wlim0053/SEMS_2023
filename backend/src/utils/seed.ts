...
// school 
const school_names = ["School of Arts and Social Sciences", "School of Business", "School of Engineering",
					"School of Information Technology", "School of Pharmacy", "School of Science"]

export const createSchool = async (schoolData: { school_name: any }) => {
	const connection = await pool.connect();
	try {
		const createSchoolSQL = await fs.readFile(sqlPath("create_school.sql"), "utf-8");
...
export const updateSchool = async (school_uuid: any, schoolData: { school_name: any }) => {
	const connection = await pool.connect();
	try {
		const updateSchoolSQL = await fs.readFile(sqlPath("update_school.sql"), "utf-8");
...
export const deleteSchool = async (school_uuid: any) => {
	const connection = await pool.connect();
	try {
		const deleteSchoolSQL = await fs.readFile(sqlPath("delete_school.sql"), "utf-8");
